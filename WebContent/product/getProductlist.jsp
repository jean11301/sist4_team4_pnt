...
<c:if test="${empty country and empty city and empty market}"> <%--나라와 도시, 시장 모두 널일때 --%>
	<sql:query var="rs" dataSource="${conn}">
		...
		WHERE country.country_code = city.country_code AND city.city_number = market.city_number AND
		market.market_number = product.market_number
		AND product_name LIKE CONCAT(CONCAT('%', ?), '%') AND check_status = 1
		ORDER BY SEQUENCE DESC
		<sql:param value="${product}" />
	</sql:query>
	<div class="container-fluid" style="min-height:0; margin:0;">
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-2" style="margin:0; padding:0"></div>
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-8" style="margin:10px; padding:0"><strong>전체 최신순</strong></div>
	</div>
</c:if>
<c:if test="${not empty country and empty city and empty market}"> <%-- 나라는 널이 아니고 도시, 시장이 널일때 --%>
	...
	<div class="container">
		...
		<c:if test="${product eq ''}">
			<p>${country}의 검색결과는 <c:forEach var="countt" items="${rs.rowsByIndex}">
					<c:set var="co" value="${co + 1}" />
				</c:forEach>
				<c:out value="${co}" />개 입니다.</p>
		</c:if>
		<c:if test="${product ne ''}">
			<p>${country}의 '${product}' 검색결과는 <c:forEach var="countt" items="${rs.rowsByIndex}">
					<c:set var="co" value="${co + 1}" />
				</c:forEach>
				<c:out value="${co}" />개 입니다.</p>
		</c:if>
...
<c:if test="${not empty country and not empty city and empty market}"> <%-- 나라, 도시 널이 아니고 시장이 널일때 --%>
	<sql:query var="rs" dataSource="${conn}">
		...
		WHERE country.country_code = city.country_code AND city.city_number = market.city_number AND
		market.market_number = product.market_number
		AND country_kr_name = ? AND city_kr_name = ? AND product_name LIKE CONCAT(CONCAT('%', ?), '%') AND check_status
		= 1
		ORDER BY SEQUENCE DESC
		<sql:param value="${country}" />
	...
	<div class="container">
		<c:set var="co" value="0" />
		<c:if test="${product eq ''}">
			<p>${country} > ${city}의 검색결과는 <c:forEach var="countt" items="${rs.rowsByIndex}">
					<c:set var="co" value="${co + 1}" />
				</c:forEach>
				<c:out value="${co}" />개 입니다.</p>
		</c:if>
		<c:if test="${product ne ''}">
			<p>${country} > ${city}의 '${product}' 검색결과는 <c:forEach var="countt" items="${rs.rowsByIndex}">
					<c:set var="co" value="${co + 1}" />
				</c:forEach>
				<c:out value="${co}" />개 입니다.</p>
		</c:if>
...
<c:if test="${not empty country and not empty city and not empty market}"><%-- 나라, 도시, 시장 셋 다 널이 아닐 때 --%>
	...
	<div class="container">
		<c:set var="co" value="0" />
		<c:if test="${product eq ''}">
			<p>${country} > ${city} > ${market}의 검색결과는 <c:forEach var="countt" items="${rs.rowsByIndex}">
					<c:set var="co" value="${co + 1}" />
				</c:forEach>
				<c:out value="${co}" />개 입니다.</p>
		</c:if>
		<c:if test="${product ne ''}">
			<p>${country} > ${city} > ${market}의 '${product}' 검색결과는 <c:forEach var="countt" items="${rs.rowsByIndex}">
					<c:set var="co" value="${co + 1}" />
				</c:forEach>
				<c:out value="${co}" />개 입니다.</p>
		</c:if>
...
<style>
	...
	.stylebox:hover{border:1px solid #064acb;box-shadow: 1px 2px 7px -4px grey;}
	.pricetest{display: inline-block;    font-size: 2rem;   color: #294e94;  font-family: 'Roboto Slab', serif;}
	.pricebox{display: inline-block;float: right;padding-right: 5px;text-align: right;}
	#won{display:inline-block;}
</style>
...
=======
				<a href="productDetail.jsp?product_number=${row.product_number}">
					<table class="table-responsive">
						...
						<tr class="product">
							<td class="productlist">${row.country_kr_name} > ${row.city_kr_name} > ${row.market_kr_name}
							</td>
						</tr>
						<tr>
							<td class="productname">${row.product_name}</td>
						...
						<tr>
							<c:if test="${row.country_kr_name eq '태국' }">
								<td>
									<div class="pricebox">
									<div id="test" class="pricetest">${row.product_price}</div>
									</div>
								</td>
							</c:if>
							<c:if test="${row.country_kr_name eq '라오스'}">
								<td>
									<div class="pricebox">
									<div id="test1" class="pricetest">${row.product_price}</div>
									</div>
								</td>
							</c:if>
							<c:if test="${row.country_kr_name eq '말레이시아'}">
								<td>
									<div class="pricebox">
									<div id="test2" class="pricetest">${row.product_price}</div>
									</div>
								</td>
							</c:if>
							<c:if test="${row.country_kr_name eq '베트남'}">
								<td>
									<div class="pricebox">
									<div id="test3" class="pricetest">${row.product_price}</div>
									</div>
								</td>
							</c:if>
							<c:if test="${row.country_kr_name eq '싱가폴'}">
								<td>
									<div class="pricebox">
									<div id="test4" class="pricetest">${row.product_price}</div>
									</div>
								</td>
							</c:if>
						</tr>
	...
	<script>
		$.ajax({
			...
			$("div#test").each(function (idx) {
				...
				$("div#test:eq(" + idx + ")").after("<div id='won'>원</div><div class='colchange'>" + test11 + "밧</div>");;
				console.log(eqValue);
			...
			$("div#test1").each(function (idx) {
				...
				$("div#test1:eq(" + idx + ")").after("<div id='won'>원</div><div class='colchange'>" + test11 + "킵</div>");;
				console.log(eqValue);
				// $('#change1').append(eqValue);                                        
			});
			...
			$("div#test2").each(function (idx) {
				...
				var test11 = Math.round((eqValue / mmm2 * e11) * 100) / 100;
				$("div#test2:eq(" + idx + ")").after("<div id='won'>원</div><div class='colchange'>" + test11 + "링깃</div>");;
				console.log(eqValue);
				// $('#change1').append(eqValue);                                        
			});
			...
			$("div#test3").each(function (idx) {
				...
				var test11 = Math.round((eqValue / mmm2 * e11) * 100) / 100;
				$("div#test3:eq(" + idx + ")").after("<div id='won'>원</div><div class='colchange'>" + test11 + "동</div>");;
				console.log(eqValue);
				// $('#change1').append(eqValue);                                        
			});
			...
			$("div#test4").each(function (idx) {
				...
				var test11 = Math.round((eqValue / mmm2 * e11) * 100) / 100;
				$("div#test4:eq(" + idx + ")").after("<div id='won'>원</div><div class='colchange'>" + test11 + "달러</div>");;
				console.log(eqValue);
				// $('#change1').append(eqValue);                                        
			});
